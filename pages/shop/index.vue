    ...
    <v-card max-width="400" outlined class="mb-2">
      <v-list-item three-line>
        <v-list-item-content>
          ...
          <v-list-item-title class="text-h5 mb-1">{{ amount }}</v-list-item-title>
      ...
      <v-card-actions>
        <v-list-item class="grow">
          <v-list-item-avatar color="grey darken-3">
            <v-img class="elevation-6" alt="peña nieto" src="https://pbs.twimg.com/profile_images/991180595710513152/V5O-Z320_400x400.jpg"></v-img>
          </v-list-item-avatar>
...
export default {
    ...
    methods: {
        async getMoney() {
            try {
                const { data } = await this.$axios.$post(`${process.env.baseUrl}/money/get`);
                return data;
            } catch (error) {
